 ...
 */
@Slf4j
public class RedisDatasourceFactory implements DatasourceFactory<RedisDatasourceInstance, Object> {
    
    // 每一个数据源ID对应一个数据源实例
    private static final Map<Long, RedisDatasourceInstance> DATASOURCE_INSTANCE_MAP = new ConcurrentHashMap<>();
    
    // 对象锁
    private static final Object LOCK = new Object();
    
    @Override
    public RedisDatasourceInstance create(DatasourceProperties config) {
        // 基于数据源配置，判断数据源是否需要进行连接池创建
        String tenantId = config.getTenantId();
        Long datasourceId = config.getDatasourceId();
        if (datasourceId == null) {
            return new RedisDatasourceInstance(config);
        }
        ...
        synchronized (LOCK) {
            if (DATASOURCE_INSTANCE_MAP.containsKey(datasourceId)) {
                ...
                RedisDatasourceInstance datasourceInstance = new RedisDatasourceInstance(config);
    ...
    public void destroy(DatasourceProperties properties) {
        ...
        RedisDatasourceInstance instance = DATASOURCE_INSTANCE_MAP.remove(datasourceId);
        if (instance == null) {
